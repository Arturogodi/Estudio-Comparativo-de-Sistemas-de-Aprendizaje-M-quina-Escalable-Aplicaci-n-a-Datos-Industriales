...
base_dir = "data/images/07_AMZN_DASK"
os.makedirs(base_dir, exist_ok=True)

# Crear subcarpeta para read_filter_write dask
rfw_dir = os.path.join(base_dir, "read_filter_write_dask")
...
for run in range(1, 6):
    ...
    filename = f"Read_Filter_Write_Dask__amzn23_model_data.parquet__run{run}.csv"
    # Crear la ruta completa
    path = os.path.join(metrics_dir, filename)
    # Crear la etiqueta usando el primer run
    run_label = f"run{run}_ReadFilterWrite_Dask"

...
for run in range(1, 6):
    ...
    filename = f"ETL_Join_Meta_+_Reviews__ETL_Join_Meta_+_Reviews__run{run}.parquet__run1.csv"
    # Crear la ruta completa
    path = os.path.join(metrics_dir, filename)
    # Crear la etiqueta usando el primer run
    run_label = f"run{run}_ETL_Join"
...
if all_dfs:
    ...
    plot_comparison_by_worker_grouped(df_concat, base_dir, "02_AMZN")

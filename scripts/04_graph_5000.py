...
base_dir = "data/images/04_AMZN_5000"
os.makedirs(base_dir, exist_ok=True)

# Crear subcarpeta para kmeans sampled
kmeans_dir = os.path.join(base_dir, "kmeans_sampled")
...
for run in range(1, 6):
    ...
    filename = f"KMeans_Clustering_Sampled__KMeans_Sampled_Amazon23__run{run}.parquet__run1.csv"
    # Crear la ruta completa
    path = os.path.join(metrics_dir, filename)
    # Crear la etiqueta usando el primer run
    run_label = f"run{run}_KMeans_Sampled"
...
for path, run_label in archivos:
    ...
    try:
        ...
        # Guardar en la carpeta kmeans_sampled
...
if all_dfs:
    ...
    plot_comparison_by_worker_grouped(df_concat, base_dir, "04_AMZN_5000")

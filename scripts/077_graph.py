...
base_dir = "data/images/077_AMZN_SPARK"
...
for run in range(1, 6):
    ...
    filename = f"Read_Filter_Write__amzn23_model_data.parquet__run{run}.csv"
    # Crear la ruta completa
    path = os.path.join(metrics_dir, filename)
    # Crear la etiqueta usando el primer run
    run_label = f"run{run}_ReadFilterWrite_Spark"
...
if all_dfs:
    ...
    plot_comparison_by_worker_grouped(df_concat, base_dir, "077_AMZN_SPARK")

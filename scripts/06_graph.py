...
base_dir = "data/images/06_AMZN"
os.makedirs(base_dir, exist_ok=True)

# Crear subcarpeta para GBT
kmeans_dir = os.path.join(base_dir, "gbt_classification")
...
for run in range(1, 6):
    ...
    filename = f"GBT_Classification_Amazon23_Fast__amzn23_model_data.parquet__run{run}.csv"
    # Crear la ruta completa
    path = os.path.join(metrics_dir, filename)
    # Crear la etiqueta usando el primer run
    run_label = f"run{run}_GBT"
...
for path, run_label in archivos:
    ...
    try:
        ...
        # Guardar en la carpeta gbt_classification
...
if all_dfs:
    ...
    plot_comparison_by_worker_grouped(df_concat, base_dir, "06_AMZN")

...
for run in range(1, 6):
    ...
    filename = f"Preprocesamiento_para_modelos__Preprocesamiento_para_modelos__run{run}.parquet__run1.csv"
    # Crear la ruta completa
    path = os.path.join(metrics_dir, filename)
    # Crear la etiqueta usando el primer run
    run_label = f"run{run}_Preprocesamiento"
...
for path, run_label in archivos:
    ...
    try:
        ...
        # Usar el directorio base ya que no tenemos categorías separadas
...
if all_dfs:
    ...
    plot_comparison_by_worker_grouped(df_concat, base_dir, "03_AMZN")

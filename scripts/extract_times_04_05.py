...
def process_file(file_path):
    """Procesa un archivo .md y extrae los tiempos de ejecución para archivos 04 y 05"""
    ...
    for i, line in enumerate(lines):
        ...
        if time_match and not re.search(r'## Tiempo total del script', line):
            ...
            if "R_O" in file_name:
                # Buscar el número de run en el nombre del archivo
                run_match = re.search(r'parquet_R_O_report.md', file_name)
                if run_match:
            ...
            # Para archivos Ray, limitar a 5 runs
                ...
                if execution_counter > 5:
                    ...
                    runs.append(f"[Run {execution_counter}]")
            # Para otros archivos (XGBoost, etc.)
            else:
                if execution_counter > 5:
                    runs.append("[Run X]")
                else:
...
def main():
    ...
    if all_results:
        ...
        # Guardar en CSV
        output_file = os.path.join(os.getcwd(), 'data', 'images', 'times_summary_04_05.csv')

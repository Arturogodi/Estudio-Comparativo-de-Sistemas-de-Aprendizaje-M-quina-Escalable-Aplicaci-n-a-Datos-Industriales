...
base_dir = "data/images/05_AMZN"
os.makedirs(base_dir, exist_ok=True)

# Crear subcarpeta para gbt fast
gbt_dir = os.path.join(base_dir, "gbt_fast")
...
for run in range(1, 6):
    ...
    filename = f"GBT_Fast_Classification__GBT_Classification_Amazon23_Fast__run{run}.parquet__run1.csv"
    # Crear la ruta completa
    path = os.path.join(metrics_dir, filename)
    # Crear la etiqueta usando el primer run
    run_label = f"run{run}_GBT_Fast"
...
for path, run_label in archivos:
    ...
    try:
        ...
        # Guardar en la carpeta gbt_fast
...
if all_dfs:
    ...
    plot_comparison_by_worker_grouped(df_concat, base_dir, "05_AMZN")

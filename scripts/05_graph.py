...
base_dir = "data/images/055_AMZN"
os.makedirs(base_dir, exist_ok=True)

# Crear subcarpeta para gbt verified purchase
gbt_dir = os.path.join(base_dir, "gbt_verified_purchase")
os.makedirs(gbt_dir, exist_ok=True)
...
for run in range(1, 6):
    ...
    filename = f"GBT_Verified_Purchase_-_Fast__amzn23_model_data_csv__run{run}.csv"
    # Crear la ruta completa
    path = os.path.join(metrics_dir, filename)
    # Crear la etiqueta usando el primer run
    run_label = f"run{run}_GBT_Verified"
...
for path, run_label in archivos:
    ...
    try:
        ...
        # Guardar en la carpeta gbt_verified_purchase
...
if all_dfs:
    ...
    plot_comparison_by_worker_grouped(df_concat, base_dir, "055_AMZN")

import os
import pandas as pd
from templates_graph_dask import (
...
base_dir = "data/images/077_AMZN_DASK"
os.makedirs(base_dir, exist_ok=True)

# Crear subcarpeta para read_filter_count dask
rfc_dir = os.path.join(base_dir, "read_filter_count_dask")
...
archivos = []
metrics_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "data", "metrics_dask"))

# Para cada run (1-5)
for run in range(1, 6):
    # Construir el nombre del archivo
    filename = f"Read_Filter_Count_Dask__amzn23_model_data.parquet__run{run}.csv"
    # Crear la ruta completa
    path = os.path.join(metrics_dir, filename)
    # Crear la etiqueta usando el primer run
    run_label = f"run{run}_ReadFilterCount_Dask"
...
if all_dfs:
    ...
    plot_comparison_by_worker_grouped(df_concat, base_dir, "07_AMZN_DASK")

...
# Orden de contenedores para Dask
ORDER = ['dask-scheduler'] + [f'dask-worker-{i}' for i in range(1, 6)] + ['dask-notebook']
...
def plot_cpu_memory_time(df, out_dir, run_label):
    ...
    subset_names = ['dask-scheduler'] + [w for w in ORDER if 'worker' in w][:2]
